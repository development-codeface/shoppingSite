...
$dbc = new Dbc;
			...
			if ($stock <> 'empty')
			{
				if ($_POST['password'] <> '')
				{
					...
					$subject = 'Order Placed';
					 ...
					 $url = "$server/phpfiles/mail/checkout_mail.php?name=$bill_name&mail=$bill_mail"; 
	...
	//<a href="http://marginfreemarketonline.com/phpfiles/user/activation.php?em='.$bill_mail.'">Click Here</a><br/><br/>
				...
				else
				{
					if (isset($_SESSION['onlineuser']) <> '')
					{
						...
						$subject = 'Order Placed';
							 ...
							 $sess_user = $_SESSION['onlineuser'];
						 $server = 'http://'.$_SERVER['SERVER_NAME'].'';
						 $url = "$server/phpfiles/mail/checkout_mail.php?name=$bill_name&user=$sess_user&mail=$bill_mail"; 
						 $message = file_get_contents($url); 
					...
					{ 
						...
						$subject = 'Order Placed';
						 ...
						 $server = 'http://'.$_SERVER['SERVER_NAME'].'';
						 $url = "$server/phpfiles/mail/checkout_mail.php?name=$bill_name"; 
						 $message = file_get_contents($url);
