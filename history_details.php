...
$select01 = "SELECT a.id as product_id,a.name,a.price,a.brand,a.author,a.offer_price,b.quantity as quantity,b.order_date,b.status,b.id as order_pdt_id,b.order_id,b.id
...
$query03 = $dbc -> query("select amount from orders where status = 1 and id = '$id'");
...
?>
	...
	<section class="content_section_main my_order_page">
			...
			<div id="content">
				<h2 class="page_title">Purchase Details</h2>
					<table>
		<thead>
		<tr>
			<th>Order ID</th>
	...
	<?php include_once('includes/myaccount_sidebar.php'); ?>	
		...
		<tr id="order_items<?php echo $array01["order_pdt_id"]; ?>">
			...
			<td data-label="Order Summary "  class="">
<img width="100" src="<?php echo $array02['thumb_image']; ?>" />
			</td>
			<td data-label="Total Items"  ><?php echo $array01['name']; ?></td>
			<td data-label="Amount" ><?php
						    						
													echo $array01['quantity'];
												   ?></td>
			<td data-label="Amount" ><?php if(!empty($array01['offer_price'])) { echo $array01['offer_price']; } else { echo $array01['price']; }?></td>
			<td data-label="Amount" ><?php echo $array01['order_date']; ?></td> 
